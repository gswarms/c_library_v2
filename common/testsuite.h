...
static void mavlink_test_interception_data(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_INTERCEPTION_DATA >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_interception_data_t packet_in = {
        93372036854775807ULL,73.0,101.0,129.0,963498504,{ 185.0, 186.0, 187.0 },113,180
    };
    mavlink_interception_data_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.timestamp = packet_in.timestamp;
        packet1.tgo = packet_in.tgo;
        packet1.miss_distance = packet_in.miss_distance;
        packet1.position_std_norm = packet_in.position_std_norm;
        packet1.substate = packet_in.substate;
        packet1.id = packet_in.id;
        packet1.target_detected = packet_in.target_detected;
        
        mav_array_memcpy(packet1.estimated_relative_position, packet_in.estimated_relative_position, sizeof(float)*3);
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_INTERCEPTION_DATA_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_INTERCEPTION_DATA_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_interception_data_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_interception_data_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_interception_data_pack(system_id, component_id, &msg , packet1.timestamp , packet1.id , packet1.tgo , packet1.miss_distance , packet1.position_std_norm , packet1.substate , packet1.target_detected , packet1.estimated_relative_position );
    mavlink_msg_interception_data_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_interception_data_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.timestamp , packet1.id , packet1.tgo , packet1.miss_distance , packet1.position_std_norm , packet1.substate , packet1.target_detected , packet1.estimated_relative_position );
    mavlink_msg_interception_data_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_interception_data_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_interception_data_send(MAVLINK_COMM_1 , packet1.timestamp , packet1.id , packet1.tgo , packet1.miss_distance , packet1.position_std_norm , packet1.substate , packet1.target_detected , packet1.estimated_relative_position );
    mavlink_msg_interception_data_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

#ifdef MAVLINK_HAVE_GET_MESSAGE_INFO
    MAVLINK_ASSERT(mavlink_get_message_info_by_name("INTERCEPTION_DATA") != NULL);
    MAVLINK_ASSERT(mavlink_get_message_info_by_id(MAVLINK_MSG_ID_INTERCEPTION_DATA) != NULL);
#endif
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_interception_data(system_id, component_id, last_msg);
}
